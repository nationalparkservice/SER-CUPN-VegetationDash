...
### Load libraries -----
packages <- c("plyr", "magrittr", "reshape2", "plotrix", "cowplot", "lubridate", "tidyverse", "scales", "stringr", "rgdal")
...
FuncFormatVeg <- function () {
  ...
  VegTemp_df <- read_csv("Data_in/VegDat.csv")
  TempCG_df <- read_csv("Data_in/FinalCG.csv")
...
FuncMapBounds <- function () {
  CUPNmap <- readOGR("Map_files/CUPN_shapefiles/CUPN.shp")
...
FuncPlotPSA <- function(PSA_df, xlabel) {
  ...
  PlotPSA <- ggplot(PSA_df, aes(x=Grp,  y=PSA)) +
...
FuncThreshMain <- function() {
  thresh_list <- list()
  cegl_bin <- read_csv("Data_in/cegl_x_bin.csv") %>%
    select(CEGL, bin = gp1)
  
  # One chart per park-metric, x-axis is year ----
  #cwd (coarse woody debris)
  cwd<-read_csv("Data_in/qry_stat_cwd_byPanelYear.csv")
  ...
  #snag LARGE DENSITY
  snag<-read_csv("Data_in/qry_snagDensity_event_panel.csv")
  ...
  #HPE species count
  hpe2<-read_csv("Data_in/qry_event_HPExotic_count_byPanelYear.csv")
  ...
  growth_mort<-read_csv("Data_in/qry_tree_eventGrowthMort_panel.csv")
...
FuncQAQCSitesMain <- function() {
  ...
  # Stand height -----
  SiteTemp_df <- read_csv("Data_in/SiteMetrics.csv")
  ...
  CWD_df <- read_csv("Data_in/CWD.csv")
  ...
  CG <- read_csv("Data_in/Community.csv")
...
FuncQAQCTreesMain <- function() {
  ...
  TreesN_df <- read_csv("Data_in/TreesNorm.csv")
  TreesQ_df <- read_csv("Data_in/TreesQAQC.csv")
  ...
  TreeCounts_df <- read_csv("Data_in/AllTrees.csv") 
  ...
  SeedSapCounts_df <- read_csv("Data_in/SeedlingsSaplings.csv")
  SeedSapCounts_df %<>%
  ...
  for (i in names(SeedSapCounts_df)[4:11]) {
    ...
    Canopy_df <- read_csv("Data_in/Densiometer.csv")
    Canopy_df %<>%
...
FuncQAQCHerbsMain <- function () {
  ...
  HerbsTemp_df <- read_csv("Data_in/Herbs.csv") %>%
...
FuncOakHickMain <- function () {
  ...
  VegRegen_df <- read_csv("Temp_out/VegDat_cleaned.csv") %>% # <<<<<<<<<<<< GENERATE AN ERROR IF THIS IS NOT AVAILABLE --ACTUALLY NEED TO MAKE SURE ALSO THAT IT'S UPDATED
  ...
  RegenSS_df <- read_csv("Data_in/OakHickSS.csv") %>%
  ...
  RegenTree_df <- read_csv("Data_in/OakHickTree.csv") %>% # tree dbh data, status=1 means alive, status=2 means dead. Some data import issue with "Snag_Height" but we are not using those data anyway in this analysis <<<<<<<<<<<<<< THROWS A WARNING
    dplyr::rename(EventID = event_name_date_calc) %>%
    left_join(VegRegen_df, by = "EventID") %>%
  ...
  OH_SSTemp_df <- RegenSS_df %>%
    filter(Plot %in% oak_hick_plots) %>%
    dplyr::mutate(CUPNseed = select_(.,"Seed5_50:Seed50_137") %>% rowSums,
                  Sap0_2.5 = select_(.,"Sap0_1:Sap1_2.5") %>% rowSums,
                  Sap2.5_10 = select_(.,"Sap2.5_5:Sap5_10") %>% rowSums) %>%
    filter(QAQCPlot == FALSE) %>%
  ...
  #   dplyr::mutate(CUPNseed_subcat = select_(.,"Seed5_15:Seed30_50") %>% rowSums) %>%
  ...
  OHImport_SSTemp_df <- RegenSS_df %>%
    ...
    dplyr::mutate(Import_1pt = select_(.,"Seed5_15:Seed15_30") %>% rowSums,
                  Import_50pt = select_(.,"Sap0_1:Sap5_10") %>% rowSums) %>%
  ...
  Burn_df <- read_csv("Data_in/BurnPlots.csv")
