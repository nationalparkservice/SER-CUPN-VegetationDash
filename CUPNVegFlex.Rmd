...
packages <- c("flexdashboard", "shiny", "knitr", "tidyverse", "magrittr", "lubridate", "gridExtra", "scales", "shinyWidgets", "plotly", "leaflet", "RgoogleMaps", "shinyBS", "plotrix", "shinyjs")
...
source('CUPNVegFlex.R') # executes the source script, loads functions that will be used by the dashboard
...
-------------------------------------
#### Instructions for processing raw data files

1. Put the raw data files as CSV files in the 'Data_in' folder of the Dashboard working directory. Valid file names are (case-sensitive): Herbs.csv, ...
...
3. When data processing is complete, you can select reports to generate by clicking the 'CREATE REPORTS' button. Reports will be saved in the 'Reports_out' folder.

#### Instructions for using this dashboard
On each page of the dashboard, click the 'ABOUT THIS PAGE' button to open a pdf document with information on user options and an explanation of the summary information presented on that page.
...
observeEvent(eventExpr = input$button_load, {
  withProgress(message = "Loading all data...", value = 0, {
    ...
    files_load = list.files(path = './Temp_out', pattern = '*.RDS') # make sure there are processed summary files in Temp_out
    
    if(length(files_load)== 0)
    {showModal(modalDialog(title = "Aaaack!", "No .RDS files in the 'Temp_out' folder!  Please import and process new data, then try again"))}
    
    validate(need(length(files_load) > 0, message = FALSE))
    
    Out_list$CUPNbounds <- if(file.exists("Map_files/CUPNmapbounds.RDS")) readRDS("Map_files/CUPNmapbounds.RDS")
    
    Out_list$SelOptions <- if(file.exists("Temp_out/sel_options.RDS")) readRDS("Temp_out/sel_options.RDS")
    
    Out_list$MapDat <- if(file.exists("Temp_out/MapDatOut.RDS")) readRDS("Temp_out/MapDatOut.RDS")
  
  Out_list$OakHick <- if(file.exists("Temp_out/OakHickOut.RDS")) readRDS("Temp_out/OakHickOut.RDS")
  
  Out_list$OakHickDat <- if(file.exists("Temp_out/OakHickDat.RDS")) readRDS("Temp_out/OakHickDat.RDS")
  
  Out_list$QAQCHerbs <- if(file.exists("Temp_out/QAQCHerbsOut.RDS")) readRDS("Temp_out/QAQCHerbsOut.RDS")
  
  Out_list$QAQCTrees <- if(file.exists("Temp_out/QAQCTreesOut.RDS")) readRDS("Temp_out/QAQCTreesOut.RDS")
  
  Out_list$QAQCSites <- if(file.exists("Temp_out/QAQCSitesOut.RDS")) readRDS("Temp_out/QAQCSitesOut.RDS")
  
  Out_list$ThreshList <- if(file.exists("Temp_out/ThreshList.RDS")) readRDS("Temp_out/ThreshList.RDS")
...
observeEvent(eventExpr = input$button_import, {
  withProgress(message = "Importing and processing data files...", value = 0, {
    ...
    if(length(dat_temp)== 0)
      {showModal(modalDialog(title = "Aaaack!", "No .csv files in the 'Data_in' folder!  Please add the data files and try again"))}
    validate(need(length(dat_temp) > 0, message = FALSE))
  ...
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "Maps"]) %in% dat_temp)) {
    ...
    FuncMapDatMain()
    Out_list$MapDat <- readRDS("Temp_out/MapDatOut.RDS")
  }
  ...
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "Map Boundaries"]) %in% map_temp)) {
    ...
    FuncMapBounds()
    Out_list$CUPNbounds <-  readRDS("Map_files/CUPNmapbounds.RDS")
  }
  ...
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "Oak-Hickory"]) %in% dat_temp)) {
    ...
    FuncOakHickMain()
    Out_list$OakHick <- readRDS("Temp_out/OakHickOut.RDS")
    Out_list$OakHickDat <- readRDS("Temp_out/OakHickDat.RDS")
    }
  ...
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "QAQC - Sites"]) %in% dat_temp)) {
    ...
    FuncQAQCSitesMain()
    Out_list$QAQCSites <- readRDS("Temp_out/QAQCSitesOut.RDS")
    }
  ...
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "QAQC - Trees"]) %in% dat_temp)) {
    ...
    FuncQAQCTreesMain()
    Out_list$QAQCTrees <- readRDS("Temp_out/QAQCTreesOut.RDS")
  }

  # QAQC - Herbs
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "QAQC - Herbs"]) %in% dat_temp)) {
    incProgress(1/10, detail = " for 'QAQC - Herbs' reports")
    FuncQAQCHerbsMain()
    Out_list$QAQCHerbs <- readRDS("Temp_out/QAQCHerbsOut.RDS")
  }

    # Thresholds
  if(all(as.character(Process_df$DataRequired[Process_df$Analysis == "Thresholds"]) %in% dat_temp)) {
    incProgress(1/10, detail = " for 'Thresholds' plots")
    FuncThreshMain()
    Out_list$ThreshList <- readRDS("Temp_out/ThreshList.RDS")
    }
...
observeEvent(eventExpr = input$button_rpts, {
  withProgress(message = "Generating reports...", value = 0, {
    ...
    if(any(c("OakHickOSI", "OakHickImport") %in%
      ...
      validate(need("OakHickDat.RDS" %in% list.files("Temp_out/"), "The 'Oak-Hickory (OSI or Importance)' report requires the cleaned data file 'OakHickDat.RDS' in 'Temp_out' folder"),
               need("OakHickOut.RDS" %in% list.files("Temp_out/"), "The 'Oak-Hickory (OSI or Importance)' report requires the summarized data file 'OakHickOut.RDS' in 'Temp_out' folder"))
      if("OakHickOSI" %in% input$sel_OakHickRpts) {
        incProgress(1/10, detail = "OakHickory - OSI")
        rmarkdown::render("./RMD_files/OakHick_OSI_Import.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/OakHickOSI", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")), params = list(type = "PartialOSI"))}
      if("OakHickImport" %in% input$sel_OakHickRpts) {
        incProgress(1/10, detail = "OakHickory - Importance")
        rmarkdown::render("./RMD_files/OakHick_OSI_Import.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/OakHickImport", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")), params = list(type = "log10ImpScore"))}
    }

    # OakHickory - Sander
    if("OakHickSander" %in% input$sel_OakHickRpts) {
      validate(need("OakHickOut.RDS" %in% list.files("Temp_out/"), "The 'Oak-Hickory - Sander' report requires the summarized data file 'OakHickOut.RDS' in Dashboard working directory"))
    incProgress(1/10, detail = "OakHickory - Sander")
    rmarkdown::render("./RMD_files/OakHick_Sander.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/OakHickSander", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")))
    }
    
    # QAQC - Sites
    if("QAQCSites" %in% input$sel_QAQCRpts) {
    validate(need("QAQCSitesOut.RDS" %in% list.files("Temp_out/"), "The 'QAQC - Sites' report requires the summarized data file 'QAQCSitesOut.RDS' in 'Temp_out' folder"))
      incProgress(1/10, detail = "QAQC - Sites")
    rmarkdown::render("./RMD_files/QAQCSites.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/QAQCSites", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")))
    }

    # QAQC - Trees
    if("QAQCTrees" %in% input$sel_QAQCRpts) {
    validate(need("QAQCTreesOut.RDS" %in% list.files("Temp_out/"), "The 'QAQC - Trees' report requires the summarized data file 'QAQCTreesOut.RDS' in 'Temp_out' folder"))
    incProgress(1/10, detail = "QAQC - Trees")
    rmarkdown::render("./RMD_files/QAQCTrees.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/QAQCTrees", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")))
    }
    
    # QAQC - Herbs
    if("QAQCHerbsMatch" %in% input$sel_QAQCRpts) {
    validate(need("QAQCHerbsDat.RDS" %in% list.files("Temp_out/"), "The 'QAQC - Herbs - Matchlist' report requires the cleaned data file 'QAQCHerbsDat.RDS' in 'Temp_out' folder"))
    incProgress(1/10, detail = "QAQC - Herbs")
    rmarkdown::render("./RMD_files/QAQCHerbsMatch.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/QAQCHerbsMatch", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")))
    }
    
    if(any(c("QAQCHerbsBase", "QAQCHerbsHiCov", "QAQCHerbsRolled") %in% input$sel_QAQCRpts)) {
    validate(need("QAQCHerbsDat.RDS" %in% list.files("Temp_out/"), "The 'QAQC - Herbs' report requires the cleaned data file 'QAQCHerbsDat.RDS' in 'Temp_out' folder"),
             need("QAQCHerbsOut.RDS" %in% list.files("Temp_out/"), "The 'QAQC - Herbs' report requires the summarized data file 'QAQCHerbsOut.RDS' in 'Temp_out' folder"))
      if("QAQCHerbsBase" %in% input$sel_QAQCRpts) {
        incProgress(1/10, detail = "QAQC - Herbs (Base)")
        rmarkdown::render("./RMD_files/QAQCHerbs.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/QAQCHerbsBase", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")), params = list(type = "HerbsBase"))}
      if("QAQCHerbsHiCov" %in% input$sel_QAQCRpts) {
        incProgress(1/10, detail = "QAQC - Herbs (HiCov)")
        rmarkdown::render("./RMD_files/QAQCHerbs.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/QAQCHerbsHiCov", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")), params = list(type = "HerbsHiCov"))}
      if("QAQCHerbsRolled" %in% input$sel_QAQCRpts) {
        incProgress(1/10, detail = "QAQC - Herbs (Rolled)")
        rmarkdown::render("./RMD_files/QAQCHerbs.Rmd", output_format = input$sel_rptformat, output_file = paste0("../Reports_out/QAQCHerbsRolled", switch(input$sel_rptformat, word_document = ".doc", pdf_document = ".pdf")), params = list(type = "HerbsRolled"))}
      }
  })
  delfiles <- list.files(path = "./Reports_out", pattern = '*.tex')
file.remove(file.path("./Reports_out", delfiles)) # remove all .tex files that were created during report generation
  showModal(modalDialog(
    title = "Done",
...
```{r Thresh_Pointplots}
renderPlot({
  validate(need(!is.null(input$sel_ThreshDisplay), message = FALSE),
  ...
  if(input$sel_ThreshDisplay %in% c("one_param", "by_cegl")) { 
    ...
    if(input$sel_ThreshDisplay=="one_park") { 
        validate(need(!is.null(input$sel_ThreshPark), message = FALSE))
...
```{r Thresh_Table}
renderTable({
    validate(need(!is.null(Out_list$ThreshTable), message = FALSE),
...
observeEvent(eventExpr = input$button_QAQCherbs_about, {
    openPDF("PDFs/About_QAQC_Herbs.pdf")})
...
renderPlot({
  validate(need(nrow(HS_df()) > 0, message = FALSE))
...
```{r RH_CDFtab}
renderTable({
  validate(need(nrow(HStab_df()) > 0, message = FALSE))
...
renderPlot({
  validate(need(nrow(HM_df()) > 0, message = FALSE))
...
renderPlot({
  if(input$sel_RTdat %in% c("Status", "Crown", "Dieback", "SnagDecay", "Vigor")) { # for categorical variables
    validate(need(nrow(RT_list()[["PSA_df"]]) > 0, message = FALSE))
    RTcateg_PlotPSA <- FuncPlotPSA(PSA_df = RT_list()[["PSA_df"]], xlabel = input$sel_RT)
    RTcateg_PlotPSA 
  } else { # for continuous variables
    validate(need(nrow(RT_list()[["FinalDat_df"]]) > 0, message = FALSE))
...
renderPlot({
  validate(need(nrow(RT_list()[["FinalDat_df"]]) > 0, message = FALSE))
  if(input$sel_RTdat %in% c("Status", "Crown", "Dieback", "SnagDecay", "Vigor")) { # for categorical variables
    RTcateg_PlotColMat <- FuncColMat(QAQC_df = RT_list()[["FinalDat_df"]][, c("Value1", "Value2")], categ_vec = RT_list()[["categ_vec"]], upper_only = TRUE, title_add = paste0("QAQC Evaluation - ", input$sel_RTdat))
    if(file.exists('Rplots.pdf')) {file.remove('Rplots.pdf')}
    replayPlot(RTcateg_PlotColMat)
    } else { # for continuous variables
    validate(need(nrow(RT_list()[["FinalDat_df"]]) > 0, message = FALSE))
...
```{r RT_Table3}
renderTable({
  validate(need(!input$sel_RTdat %in% c("Status", "Crown", "Dieback", "SnagDecay", "Vigor"), message = "No additional plots for categorical variables"),
